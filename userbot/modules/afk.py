...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf Baginda {ALIVE_NAME} Sedang BAHAGIA!**",
    f"**Maaf Baginda {ALIVE_NAME} Sedang BAHAGIA🌈\n Tunggu Sampai Dia Kembali Online!**",
    f"**Baginda {ALIVE_NAME} Sedang BAHAGIA🌈\n Tunggulah Sampai Online**",
    f"**Maaf Baginda {ALIVE_NAME} Sedang BAHAGIA!**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**💀 BAHAGIA!**\n**Baginda Telah BAHAGIA**\
        \n🌈 **Penyebab kebahagiaan:** `{string}`")
    else:
        await afk_e.edit("**💀 BAHAGIA!**\n**Baginda Telah BAHAGIA**")
    if user.last_name:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name=user.last_name + "【BAHAGIA🌈】"))
    else:
        await afk_e.client(UpdateProfileRequest(first_name=user.first_name, last_name="【BAHAGIA🌈】"))
    if BOTLOG:
        await afk_e.client.send_message(BOTLOG_CHATID, "#BAHAGIA\n**Baginda Telah BAHAGIA!**")
    ISAFK = True
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**💀 Baginda {ALIVE_NAME} Sedang BAHAGIA** {afk_since} **Yang Lalu.**\
                        \n🌈 **Penyebab kebahagiaan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**💀 Baginda Masih BAHAGIA** {afk_since} **Yang Lalu.**\
                            \n🌈 **Penyebab kebahagiaan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"💀 **Baginda Sedang BAHAGIA** {afk_since} **Yang Lalu**.\
                        \n🌈 **Penyebab kebahagiaan**: `{AFKREASON}`")
                else:
