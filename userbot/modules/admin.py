...
NO_ADMIN = "`Mohon Sadar Diri:)`"
NO_PERM = "`Gaada Izin Mana Bisa_-`"
NO_SQL = "`Berjalan Pada Mode Non-SQL`"

CHAT_PP_CHANGED = "`Berhasil Mengubah Profil Grup`"
CHAT_PP_ERROR = (
    "`Ada Masalah Dengan Memperbarui Foto,`"
    "`Mungkin Karna Pakdhe Bukan Admin,`"
...
async def set_group_photo(gpic):
    if not gpic.is_group:
        await gpic.edit("`Pakdhe Mohon Lakukan Perintah Ini Di Grup Yaaa.`")
        return
...
async def rm_deletedacc(show):
    ...
    # Well
    if not admin and not creator:
        return await show.edit("`Pakdhe Bukan Admin Disini!`")
...
async def get_usersdel(show):
    ...
    try:
        ...
        await show.edit(
            "Pakdhe, Grup Ini Terlalu Besar, Mengunggah Daftar Akun Terhapus Sebagai File."
        )
        file = open("daftarpengguna.txt", "w+")
        file.write(mentions)
        file.close()
...
async def get_bots(show):
    ...
    try:
        ...
        await show.edit("Pakdhe, Terlalu Banyak Bot Di Grup Ini, Mengunggah Daftar Bot Sebagai File.")
